...
Date: 2026-08-16
Evidence run: GitHub Actions Engram CI `31940146076`
Result: **PASS**
...
7. The memory-free same-context control used `COMPAT_MODE`, completed `PARTIAL`, and required rollback.
8. The treatment used the exposed current memory to change the application strategy to `STAGED_CURRENT`.
9. Engram recorded the current memory as `CHANGED_ACTION` influence with the real control execution as `CONTROL_RUN` counterfactual evidence.
