...
function ExperimentCard({ experiment }: { experiment: ExperimentEvidence }) {
  const metricEntries = Object.entries(experiment.metrics ?? {});
  return <article className="experiment-card">
    <div className="experiment-top"><span>{experiment.id}</span><em>{experiment.status}</em></div>
    <h3>{experiment.title}</h3>
    <p>{experiment.claim_scope ?? "Claim scope unavailable."}</p>
    <div className="experiment-meta"><span>{experiment.kind}</span><span>{experiment.coverage ?? "UNKNOWN COVERAGE"}</span></div>
    {metricEntries.length > 0 && <dl>{metricEntries.map(([key, metric]) => <div key={key}><dt>{key.replaceAll("_", " ")}</dt><dd>{typeof metric === "object" ? JSON.stringify(metric) : String(metric)}</dd></div>)}</dl>}
    {(experiment.warnings ?? []).map((warning) => <div className="warning" key={warning}>{warning}</div>)}
  </article>;
}
...
export function EvidenceOverview({ index, sync, error }: { index: EvidenceIndex | null; sync: EvidenceSyncState; error: string | null }) {
  ...
  return <section className="product-section evidence-section" id="evidence">
    ...
    <div className="provenance-bar">
      <div><span>Source state</span><strong>{sync}</strong></div>
      <div><span>Repository</span><strong>{value(index?.repository)}</strong></div>
      <div><span>Ref</span><strong>{value(index?.ref)}</strong></div>
      <div><span>Commit</span><strong>{index?.commit?.slice(0, 12) ?? "UNAVAILABLE"}</strong></div>
    </div>

    {sync === "LOADING" && <div className="evidence-empty"><span>LOADING</span><h3>Reading canonical evidence index…</h3></div>}
    {sync === "UNAVAILABLE" && <div className="evidence-empty"><span>EVIDENCE SOURCE UNAVAILABLE</span><h3>No canonical evidence index is available to the product.</h3><p>{error ?? "The interface remains usable without fabricating evidence."}</p></div>}

    {index && <>
      <div className="dataset-grid">
        <div className="dataset-title"><span>DATASET</span><h3>{value(dataset?.benchmark)}</h3><p>{value(dataset?.tier)} / {value(dataset?.coverage)} / {dataset?.multimodal === true ? "multimodal" : dataset?.multimodal === false ? "single modality" : "UNAVAILABLE"}</p></div>
