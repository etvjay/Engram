...
const baseIndex = {
  ...
  experiments: [
    ...
    {
      id: "PRISM-13-A1",
      title: "Flat-memory control",
      kind: "CONTROL",
      status: "TESTED",
      coverage: "SUBSET",
      ablation_stage: "A1",
      claim_scope: "non-Hydra flat memory control",
    },
    {
      id: "A4",
      title: "Behavioral causal proof",
      kind: "CAUSAL",
      status: "NOT_RUN",
      coverage: "SUBSET",
      ablation_stage: "A4",
      claim_scope: "memory-off versus memory-on behavioral causal experiment",
    },
    {
      id: "PRISM-14",
      title: "Structural causal provenance",
      kind: "CAUSAL",
      status: "TESTED",
      coverage: "SUBSET",
      claim_scope: "structural causal provenance only; not behavioral causal proof",
    },
  ],
};
...
describe("evidence index contract", () => {
  ...
  it("resolves ablations only from explicit ablation_stage metadata", () => {
    ...
    expect(findAblationExperiment(parsed, "A1")?.id).toBe("PRISM-13-A1");
    expect(findAblationExperiment(parsed, "A4")?.id).toBe("A4");
  });
